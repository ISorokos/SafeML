...
DayNum = 1; % Select A Day based on Above Numbers
...
for kk = 1:PermNum
    ...
    for ii = 4:numel(classifier)
        ...
        for jj = 1:ClassNum
            ...
            Results(jj, 3, kk) = Wasserstein_Dist(XTrain_L,XTest_L);

            Results(jj, 4, kk) = Anderson_Darling_Dist(XTrain_L,XTest_L);

            Results(jj, 5, kk) = Kolmogorov_Smirnov_Dist(XTrain_L,XTest_L);

            Results(jj, 6, kk) = Kuiper_Dist(XTrain_L,XTest_L);

            Results(jj, 7, kk) = Cramer_Von_Mises(XTrain_L,XTest_L);
...
function Plots_Acc_Dist2(Results, FigNum)
    ...
    Methods = {'Chernoff','Wasserstein','Anderson-Darling','Kolmogorov-Smirnov','Kuiper','Cramer-Von Mises'};
...
function BoxPlot_Acc_Dist(SResults, FigNum, ClassNum)
    ...
    for ii = 1:6
        ...
        boxplot(x,g)
        title(Methods{ii})
        xlabel('Mean Accuracy (100 Permutation)')
        ylabel('Distance')
    end
end
