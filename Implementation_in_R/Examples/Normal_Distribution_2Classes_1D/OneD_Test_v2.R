...
# Test 2
# df2 = data.frame(
#   LBs = factor(rep(c("1", "2"), each=100)),
#   D_Val = c(rnorm(100, mean=3, sd=1),rnorm(100, mean=5, sd=1)),
#   x = seq(1,200,1)
# )
...
df = data.frame(X = df2$D_Val, LBs = factor(df2$LBs))
...
# jpeg("DensityfeaturePlot_xor.jpg")
...
a_lda = dataset[which(dataset$LBs == "1"),1]

a_cart = dataset[which(dataset$LBs == "1"),1]

a_rf = dataset[which(dataset$LBs == "1"),1]

a_knn = dataset[which(dataset$LBs == "1"),1]

a_svm = dataset[which(dataset$LBs == "1"),1]
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# b_lda = validation$X[which(predictions_lda == 1)]
b_lda = validation[which(predictions_lda == "1"),1]

b_cart = validation[which(predictions_cart == "1"),1]

b_rf = validation[which(predictions_rf == "1"),1]

b_knn = validation[which(predictions_knn == "1"),1]

b_svm = validation[which(predictions_svm == "1"),1]
